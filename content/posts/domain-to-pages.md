...
나중에 이 블로그를 같은 도메인 아래에 붙일 때 그 차이가 한 번 더 드러나는데,
그건 마지막에 적겠습니다.
...
**정작 필요했던 것은 DNS 기능 하나였고, 그게 다음 절의 이야기입니다.**
...
1절에서 DNS 기능을 보고 골랐다고 한 게 이것입니다.
...
IP 가 나오면 된 것이고, `SOA` 만 돌아오면 **그 이름은 있는데 레코드가 없는** 것입니다.
...
이 도메인을 받을지는 저장소 안의 `CNAME` 파일이 정합니다** — 바로 위에서 본 그 둘입니다.
