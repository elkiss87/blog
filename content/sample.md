...
# 주소로는 열리므로 /sample/ 로 언제든 확인할 수 있다.
...
sitemap:
  disable: true
# 첫 글에서 이 문서로 링크가 걸리면서 크롤러가 따라올 길이 생겼다.
# 읽으라고 쓴 글이 아니므로 검색 결과에서는 뺀다. 링크로는 그대로 열린다.
params:
  noindex: true
---
...
삽화는 배경이 투명한 PNG 를 넣고 빌드할 때 WebP 로 줄인다.
여기서 볼 것은 **다크 화면에서 그림이 밝은 덩어리로 뜨지 않는지**다.
배경은 비어 있어도 옷이나 종이의 흰색은 그대로 밝다.

캡션이 붙은 그림이다.

![아테나와 지휘관, 묻는 쪽과 답하는 쪽](images/athena-and-commander.png "묻는 쪽과 답하는 쪽. 삽화 규칙은 docs/ILLUST.md 에 있다")

캡션이 없으면 `<figure>` 없이 `<img>` 하나만 나가야 한다.
아래는 같은 그림을 캡션 없이 넣은 것이고, 위와 여백이 어떻게 다른지 본다.

![아테나와 지휘관](images/athena-and-commander.png)

문단 한가운데에 그림을 넣으면 `<figure>` 로 감싸지 않고 그 자리에 그대로 놓인다.
여기서 재지는 않는다 — 본문 크기의 그림을 글줄에 끼우면 문장이 갈라져서,
**쓰지도 않을 경우를 재느라 자가 지저분해진다.**
작은 아이콘을 글에 섞어 쓸 일이 생기면 그때 이 칸을 만든다.
