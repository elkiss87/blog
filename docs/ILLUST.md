...
**지휘관이 궁금해하고, 아테나가 알아본다.** 이 블로그가 실제로 굴러가는 방식이 그대로 역할이 됐다.

**삽화 속의 아테나는 궁금해하는 얼굴로 그린다.** 실제로 의문을 꺼내는 것은 지휘관인데,
그림에서는 아테나가 갸웃한다 — 읽는 사람 대신 궁금해하는 자리이기 때문이다.
어긋나 보이지만 같은 일이다. **알아보려면 먼저 궁금해해야 한다.**

| | 아테나 | 지휘관 |
|---|---|---|
| 하는 일 | 알아보고 가져온다 | 궁금해하고, 무엇이 맞는지 정한다 |
...
**그림체를 하나로 묶지 않는다** (2026-09-22). 지금까지는 연필 선화 + SD 2.5등신이
사실상 고정이었는데, 세 번째 글의 커버를 네 번 뽑는 동안 배경이 끝내 투명으로 안 나왔고
**배경이 있는 그 판도 나쁘지 않았다.** 기준을 "같은 그림체" 에서 **"얼추 비슷하면 된다"**
로 내린다. 등신대와 배경은 컷마다 열어둔다 — SD 가 아니어도 되고 배경이 있어도 된다.

**정정 (2026-09-23)** — 위의 "배경이 끝내 투명으로 안 나왔다" 는 틀렸다. 그림을 보던
미리보기가 투명을 검게 깔아서 비네팅처럼 보였을 뿐이고, 알파로 재면 어두운 배경은 없었다 → §12.
**방침은 그대로 둔다.** 지휘관이 여러 그림체를 원한 것이 본래 이유였고, 그건 착시와 상관이 없다.

- **안 바뀌는 것은 둘이다.** 회색조 + 캐릭터별 스팟 컬러 하나(§2), 그리고 글자를 넣지 않는 것
- **감수하는 비용** — 배경이 있는 그림은 라이트나 다크 한쪽에서 판처럼 보인다.
  그래서 §4 의 "투명으로 받는다" 는 **기본값으로 남긴다.** 안 되면 그대로 쓰되,
  실제 화면에서 양쪽을 보고 거슬리면 그 컷만 다시 뽑는다
...
## 10. 삽화의 양 — 커버는 의무, 본문은 선택

**축이 둘이다** (2026-08-21 에 나눴다).
표의 본문 삽화 칸은 처음에 "필요할 때만, 한두 장" 이었는데 이틀 뒤 아래 「몇 장인가」에서
개수로 세지 않기로 바꾸고도 칸을 안 고쳤다. 2026-09-26 에 그 칸을 믿고 "한두 장 찼다" 고
잘못 판단한 일이 있어서 맞췄다.

| | 커버 | 본문 삽화 |
|---|---|---|
| 하는 일 | 글의 **얼굴** | 그 대목의 **설명** |
| 규칙 | **글마다 하나. 예외 없다** | 마른 구간으로 센다 → 아래 「몇 장인가」 |
| 쓰이는 곳 | 목록 · 글 상단 · `og:image` | 본문 안 |
| 만드는 비용 | 낮다 | 높다 |

**커버가 싼 이유는 설명할 필요가 없어서다.** 아테나가 주제와 관련된 무언가를 들고
있으면 된다. A·B·C 블록이 고정돼 있으니 D·E 두 문단만 갈아 끼우면 한 장이 나온다.

**커버는 투명 그대로 둔다.** 배경을 채워야 하는 곳은 `og:image` 하나뿐이고,
그건 빌드가 알아서 한다 — jpg 로 내보내면서 `#faf9f7` 로 채운다.
목록에서는 카드 배경이 캐릭터 좌우의 빈 자리를 받아준다 → [PLAN.md](PLAN.md) §5.

**커버용 컷은 프레임을 채우는 구도가 낫다.** 캐릭터 단독 컷은 세로로 길어서
가로 3:2 에 넣으면 좌우가 거의 다 빈다. `athena-published` 처럼 배경 요소가
하나 있으면 자연스럽게 찬다. 카드가 빈 자리를 받아주므로 급한 문제는 아니다.
...
첫 글이 정확히 그 문제였다 — 두 장이 있는데도 심심했고, 자리가 몰려서였다.

**기준은 재서 나왔다.** 첫 글의 마른 구간이 1021자와 962자 둘이었고 나머지는 전부
520자 이하였다. 심심하다고 느낀 자리가 그 둘과 그대로 겹쳤다 → [LOG.md](LOG.md) 2026-08-23.

**쉼표가 꼭 삽화일 필요는 없다.** 표도 코드도 인용도 같은 일을 한다.
본문 삽화는 위에 적은 대로 비싸므로 **먼저 표나 코드로 풀리는지 보고, 그림이라야
할 때만 그림이다.** 첫 글에서는 한 자리가 그림(원본과 사본을 한 장면에 놓아야 했다),
한 자리가 표(생성기별로 뜻이 읽히는지)로 갈렸다. 외주는 한 장만 늘었다.

**고치고 나니 구간이 589 / 520 / 515 / 462 / 462 로 고르게 내려왔다.**
...
| `athena-rule-plaque.png` | 세 번째 글 「둘 다 맞는 선택이었습니다」 | 2026-09-26 | 2.54MB |
...
14% 로 도입 컷(11%) 쪽에 가까웠다. **넓은 것과 밝은 것이 다르다** —
게시판이 선으로만 그려져서 면적을 안 채운다.
...
F 의 포인트 지정을 갈아 끼워야 하는데, **손으로 조립하다 한 글자만 달라져도 다른 그림이
나온다.** 어긋난 문구까지 포함해서 그 전문이 그 그림을 만들었기 때문이다.

**전문을 이 문서에 넣지 않는 이유는 중복이 어긋나기 때문이다.** 캐릭터 설정을 고치면
블록과 전문 두 군데를 고쳐야 하고, 한쪽만 고치면 그때부터 문서가 거짓말을 한다.
...
PLAN §5 가 "페이지는 셋으로 시작한다" 다. 갤러리는 네 번째 페이지다.
**2026-09-06 기준 여섯 장인데 여섯 장 다 글이나 소개에 실려 있다** — 조건의 뒷부분
("글에서 보여줄 자리가 없을 때")이 아직 하나도 안 맞는다. 쓰지 않고 남은 컷이 없다.
